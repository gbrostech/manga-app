...
def process_mangas(input_json_path, limit):
    ...
    for i, manga in enumerate(mangas_list, 1):
        print(f"Procesando manga {i}/{len(mangas_list)}: {manga['manga']}")
...
# Función para limpiar los títulos de volúmenes
def clean_volume_titles(volumes_json_path):
    """Limpia los títulos de volúmenes para eliminar información adicional."""
    print("Limpiando títulos de volúmenes...")
    ...
    for volume in volumes:
        # Extraer solo el título principal
        title = volume["titulo"]
        match = re.match(r'^(.*?)(?:\d+\s*páginas|\d+\s*€)', title)
        if match:
            volume["titulo"] = match.group(1).strip()
    
    # Guardar el JSON actualizado
    with open('volumes_fixed.json', 'w', encoding='utf-8') as f:
        json.dump(volumes, f, ensure_ascii=False, indent=2)
    
    print(f"Títulos de volúmenes limpiados y guardados en volumes_fixed.json")
    return True
